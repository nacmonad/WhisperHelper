...
# WhisperHelper - A macOS utility for system-wide speech-to-text
...
check_dependencies() {
  ...
  # macOS doesn't use xdotool or xclip, check for pbcopy instead
...
start_recording() {
  ...
  if ! kill -0 "$RECORDING_PID" 2>/dev/null; then
    ...
    show_notification "WhisperHelper" "Recording failed to start"
    rm -f "$LOCK_FILE"
...
stop_recording() {
  ...
  if [ -f "$TEMP_AUDIO_FILE" ]; then
    if [ -s "$TEMP_AUDIO_FILE" ]; then
      ...
      show_notification "WhisperHelper" "Transcribing audio..."
      ...
      if [ $transcription_status -eq 0 ] && [ -f "$TEMP_TRANSCRIPT_FILE" ]; then
        ...
        if [ ! -s "$TEMP_TRANSCRIPT_FILE" ]; then
          ...
          insert_text
          show_notification "WhisperHelper" "Transcription inserted"
        fi
      else
      ...
      show_notification "WhisperHelper" "No audio recorded (empty file)"
    fi
  else
    log_debug "No audio file found at: $TEMP_AUDIO_FILE"
    echo "No recording found to process or file is empty." >&2
    show_notification "WhisperHelper" "No audio recorded (missing file)"
  fi
...
insert_text() {
  ...
  log_debug "Copying text to clipboard"
  # On macOS we use pbcopy instead of xdotool/xclip
  echo "$text" | pbcopy
  log_debug "Text copied to clipboard"
  
  # Also simulate typing if requested by setting TYPE_TEXT=true in environment
  if [ "$TYPE_TEXT" = "true" ]; then
    log_debug "Attempting to simulate typing with AppleScript"
    osascript -e "tell application \"System Events\" to keystroke \"$text\""
    log_debug "Text entered via AppleScript"
    echo "Text entered in active application" >&2
  else
    echo "Text copied to clipboard" >&2
    # Notify the user that text is in clipboard
